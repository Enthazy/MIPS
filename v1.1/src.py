            ...
            for idx_j in prange(len(points) - idx_i - 1):
                ...
                if root62n < x_diff < root62:  # close enough in x direction
                    ...
                    if root62n < y_diff < root62:  # close enough in y direction
                        ...
                        if d2 < root64:  # distance smaller than 2^1/3
                            # =========================================================
...
@jit(nopython=True)
def q_gradient_calculator(qx, qy, qtheta,
                          px, py,
                          grid, Pe, M, Lx, Ly):
    """
...
@jit(nopython=True)
def euler_updater(qx, qy, qtheta, px, py,
                  ax, ay,
                  s_x, s_y, s_theta,
    ...
    dqx, dqy = q_gradient_calculator(qx, qy, qtheta,
                                   px, py,
                                   grid, Pe, M, Lx, Ly)

    ax += step * dqx + sqrt2 * sqrt_step * s_x
    ay += step * dqy + sqrt2 * sqrt_step * s_y
    qtheta += sqrt6 * sqrt_step * s_theta
    return ax, ay, qtheta


@jit(nopython=True)
def run(qx, qy, qtheta, px, py,
        ax, ay,
        s_x, s_y, s_theta,
        grid, step, Pe, M, Lx, Ly):

    ax, ay, qtheta = euler_updater(qx, qy, qtheta, px, py,
                                   ax, ay,
                                   s_x, s_y, s_theta,
                                   grid, step, Pe, M, Lx, Ly)
    ...
    # qtheta = np.remainder(qtheta, 2 * np.float64(np.pi)).astype(np.float64)
