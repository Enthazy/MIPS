...
def display_circle(posx, posy):
    fig, ax = plt.subplots(1, 1, figsize=(8, 8))
    # Now, we draw our points with a gradient of colors.
    circles(posx[:len(posx)//2], posy[:len(posx)//2], 1/2, c='teal')
    circles(posx[len(posx)//2:], posy[len(posx)//2:], 1/2, c='coral')
    ax.axis('equal')
    ax.grid()
...
def generate_gif(folder_name):
    import os
    data = []
    from natsort import os_sorted, humansorted, realsorted
